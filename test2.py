...
xs = torch.concat((xs, xs+0.001))
ys = function(xs).to(device)

# create nns
trained_nns =[train_nn(xs, ys, maximum_gradient_steps=100_000, minimum_loss=0.000001, activation_function=torch.nn.ReLU) for i in range(number_nns)]
